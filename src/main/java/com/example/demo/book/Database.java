...
public class Database {
    ...
    public List<Book> searchDocuments(String keyword) {
        ...
        try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
            ...
            ResultSet rs = pstmt.executeQuery();
            if (!rs.next()) {
                books = searchDocumentsAPI(keyword);
            }
            else {
                while (rs.next()) {
                    books.add(new Book(
    ...
    public List<Book> searchDocumentsAPI(String keyword) {
        List<Book> books = new ArrayList<>();
        System.out.println("No data returned from database so trying to connect api source.");
        books = ApiService.searchBooks(keyword);
        if(!books.isEmpty()) {
    ...
    public void updateBookDatabase(List<Book> books) {
        String insertSQL = "INSERT INTO books (ISBN, Title, Author, PublishYear, Publisher, `Image-URL-M`, quantity, rating) " +
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        try (PreparedStatement preparedStatement = connection.prepareStatement(insertSQL)) {
            ...
            for (Book book : books) {
                double rating = 5 + (5 * random.nextDouble());

                preparedStatement.setString(1, book.getISBN());
                preparedStatement.setString(2, book.getTitle());
                preparedStatement.setString(3, book.getAuthor()); // Author
                preparedStatement.setString(4, book.getPublishYear());
            ...
            System.out.println("Books successfully inserted into the database!");

        } catch (SQLException e) {
            e.printStackTrace();
            System.err.println("An error occurred while updating the database.");
        }
    }
